...
class Solution(object):
    def maxProfit(self, k, prices):
        ...
        for _k in range(k+1):
            if _k == 0:
                dp[0][_k][0], dp[0][_k][1] = 0, -prices[0]
            else:
                dp[0][_k][0], dp[0][_k][1] = -maxint, -maxint
        ...
        for i in range(1, len(prices)):
		    ...
		    dp[i][_k][0] = max(
                        dp[i-1][1][0],
                        dp[i-1][_k-1][1]+prices[i]
                    )
            	    dp[i][_k][1] = max(
                        dp[i-1][_k][1],
                        dp[i-1][_k][0]+(-prices[i])
                    )
		res = max(res, dp[i][_k][0], dp[i][_k][1])
        #print dp   
        return res
...
if __name__ == '__main__':
    obj = Solution()
    print obj.maxProfit(2, [1,2,3])
    print obj.maxProfit(2, [3,3,5,0,0,3,1,4])
    print obj.maxProfit(2, [1,2,3,4,5])
    print obj.maxProfit(2, [7,6,4,3,1])
    pass
